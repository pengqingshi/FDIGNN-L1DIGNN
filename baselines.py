...
class RD_GCN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, edge_weight=None):
        x = RDLayer_ADMMAP_baselines(x, edge_index, edge_weight, rho=self.rho, mu_k_max=self.mu_k_max, mu_k=self.mu_k, K_iter=self.K_iter)
        x = F.relu(self.conv1(x, edge_index, edge_weight))
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.conv2(x, edge_index, edge_weight)
        return F.log_softmax(x, dim=1)
    
class RD_GAT(torch.nn.Module):
    def __init__(self, 
                 in_channels, 
                 out_channels, 
                 num_hid=8,
    ...
    def forward(self, x, edge_index, edge_weight=None):
        x = RDLayer_ADMMAP_baselines(x, edge_index, edge_weight, self.rho, self.mu_k_max, self.mu_k, self.K_iter)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = F.elu(self.conv1(x, edge_index, edge_weight))
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.conv2(x, edge_index, edge_weight)
...
class RD_APPNP(torch.nn.Module):
    def __init__(self,
                 in_channels, 
                 out_channels,
                 num_hid=16,
    ...
    def forward(self, x, edge_index, edge_weight=None):
        x = RDLayer_ADMMAP_baselines(x, edge_index, edge_weight, rho=1.1, mu_k_max=10e6, mu_k=1, K_iter=10)
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = F.relu(self.lin1(x))
        x = F.dropout(x, p=self.dropout, training=self.training)
        x = self.lin2(x)
