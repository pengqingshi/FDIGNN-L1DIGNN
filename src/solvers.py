...
def ADMM(x, Delta_Laplacian, rho, mu_k_max, mu_k, K_iter, edge_index):
    num_nodes = x.size(0)
    row, col = edge_index
    H = x
    Z_k = x
    ...
    mu_k_max = torch.tensor(mu_k_max, dtype=torch.float)
    mu_k = torch.tensor(mu_k, dtype=torch.float)

    k_iter = 0

    def soft_threshold_operator(x):
        return torch.sign(x) * torch.relu(torch.abs(x) - mu_k)

    while k_iter < K_iter:
        E_k = soft_threshold_operator(Z_k + 1 / mu_k * Lambda_k - H)

        EHL = E_k + H - Lambda_k / mu_k
        Z_k = EHL - scatter_add(1/ mu_k * Delta_Laplacian * (EHL[row, :] - EHL[col, :]), row, dim=0, dim_size=num_nodes)

        Lambda_k = Lambda_k + mu_k * (Z_k - E_k - H)
