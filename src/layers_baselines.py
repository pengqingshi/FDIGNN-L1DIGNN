...
def RDLayer_ADMMAP_baselines(x, edge_index, edge_weight, rho, mu_k_max, mu_k, K_iter):
    device = x.device
    num_nodes = x.size(0)
    ...
    A = torch.sparse_coo_tensor(edge_index, edge_weight, (num_nodes, num_nodes), device=device)


    degree = scatter_add(edge_weight, edge_index[0], dim=0, dim_size=num_nodes)
    D_inv_sqrt_values = degree.pow(-0.5)
    D_inv_sqrt_values.masked_fill_(D_inv_sqrt_values == float('inf'), 0)
    D_inv_sqrt = torch.sparse_coo_tensor(torch.arange(num_nodes).unsqueeze(0).repeat(2, 1), D_inv_sqrt_values, (num_nodes, num_nodes), device=device)

    I = torch.eye(num_nodes, device=device)
    L_normalized = I - torch.sparse.mm(torch.sparse.mm(D_inv_sqrt, A), D_inv_sqrt).to_dense()

    # Initialization
    Z_k = x
    Lambda_k = torch.zeros_like(x)
    mu_k_max = torch.tensor(mu_k_max, dtype=torch.float, device=device)
    mu_k = torch.tensor(mu_k, dtype=torch.float, device=device)

    # Soft threshold operator
    def soft_threshold_operator(x):
        return torch.sign(x) * F.relu(torch.abs(x) - mu_k)
    
    # Iterate ADMM
    for k_iter in range(K_iter):
        E_k = soft_threshold_operator(Z_k + 1/mu_k * Lambda_k - x)

        Z_k = torch.mm(1/mu_k * (I - 1/mu_k * L_normalized),  (mu_k * (E_k + x) - Lambda_k))

        Lambda_k = Lambda_k + mu_k * (Z_k - E_k - x)

        mu_k = torch.min(mu_k_max, rho * mu_k)
